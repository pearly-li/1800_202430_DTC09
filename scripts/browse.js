...
var filterEventSearchKeyword = document
...
async function clickFilterButton() {
  document.getElementById(
    "browsing_list"
  ).innerHTML = `<p class="mx-auto py-40">Loading...</p>`;
  ...
  filterEventSearchKeyword = document
    .getElementById("default-search")
    .value.toLowerCase()
    .split(" ");
...
async function filterResults() {
  ...
  for (let docID of eventBrowsingList) {
    ...
    if (
      filterEventCategory != "" &&
      filterEventCategory != doc.data().category
    ) {
      continue;
    }

    if (
      filterEventSize == "one on one" &&
      doc.data().maximumParticipants != 1
    ) {
      continue;
    } else if (
      filterEventSize == "small" &&
      (doc.data().maximumParticipants < 2 || doc.data().maximumParticipants > 6)
    ) {
      continue;
    } else if (
      filterEventSize == "medium" &&
      (doc.data().maximumParticipants < 7 ||
        doc.data().maximumParticipants > 12)
    ) {
      continue;
    } else if (
      filterEventSize == "large" &&
      doc.data().maximumParticipants < 13
    ) {
      continue;
    }
    ...
    if (filterEventActivityLevel == "low" && eventLevel != 1) {
      ...
      filterEventActivityLevel == "low-medium" &&
      (eventLevel < 2 || eventLevel > 3)
    ) {
      continue;
    } else if (
      filterEventActivityLevel == "medium" &&
      (eventLevel < 4 || eventLevel > 6)
    ) {
      continue;
    } else if (
      filterEventActivityLevel == "high-medium" &&
      (eventLevel < 7 || eventLevel > 9)
    ) {
      continue;
    ...
    if (filterEventDate != "") {
      ...
      eventDateTimeEachComponent = getDateList(
        new Date(`${filterEventDate}, 0:00`)
      );

      if (
        !(
          dateEachComponent.year == eventDateTimeEachComponent.year &&
...
function displayResults() {
  if (eventBrowsingListSize != 0) {
    ...
    for (index; index < maxIndex; index++) {
      ...
      db.collection("events")
        ...
        .then((doc) => {
          ...
          dateDisplay = checkIfTodayOrTomorrowOrYesterday(
            todayEachComponent,
          ...
          newCard.querySelector(".event_card_time").innerHTML = formatTimeAgo(
            dateDisplay,
            todayEachComponent,
            dateEachComponent
          );
    ...
    document.getElementById(
      "browsing_list"
    ).innerHTML = `<p class="mx-auto my-20">No Events Found.</p>`;
  }
}
